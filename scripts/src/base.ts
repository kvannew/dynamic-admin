...
import { isArray, isString } from '@dynamic-admin/utils';
...
async function baseScript(command: string, isFilterWorkspace: boolean) {
  ...
  try {
    if (isArray(argv)) {
      filterArgv = argv.map((argvItem) => ['--filter', `../${argvItem}`]).flatMap((argvItem) => argvItem);
    ...
    const scriptArgv = isArray(packages)
      ? packages.map((argvItem) => ['--no-daemon', '--filter', argvItem]).flatMap((argvItem) => argvItem)
